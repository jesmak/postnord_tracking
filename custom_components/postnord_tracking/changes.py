...
The account is polled, so a single update can carry several changes: one package arrives at a
pickup point while another is delivered. Each becomes an event of its own, so an automation sees
them all.
"""
...
from .shipments import STATUS_DELIVERED, STATUS_READY_FOR_PICKUP, STATUS_RETURNED
...
EVENT_BY_STATUS = {
    ...
    STATUS_RETURNED: EVENT_RETURNED,
}
...
def detect_changes(
    ...
    Nothing is reported for the first update: the packages that are already there when Home
    Assistant starts have not just happened.
    """
    if previous is None:
        return []
